    ...
    def add_to_nwbfile(self, nwbfile: NWBFile, metadata: dict):
        with h5py.File(name=self.source_data["v1_nwbfile_path"], mode="r") as v1_nwbfile:
            ...
            for (source_name, image_series_name), presentation_name in zip(
                ...
                # Template
                natural_movie_template_source = v1_nwbfile["stimulus"]["templates"][source_name]

                natural_movie_data = natural_movie_template_source["data"]
                max_frames_per_chunk = int(
                    10e6
                    / (natural_movie_data.dtype.itemsize * natural_movie_data.shape[1] * natural_movie_data.shape[2])
                )
                movie_chunks = (
                ...
                # Presentation
                natural_movie_presentation_source = v1_nwbfile["stimulus"]["presentation"][presentation_name]
                ...
                natural_movie_presentation_data = numpy.array(natural_movie_presentation_source["data"], dtype="uint16")
                max_frames_per_chunk = int(10e6 / natural_movie_presentation_data.dtype.itemsize)
                natural_movie_presentation_data_chunks = min(
                    natural_movie_presentation_data.shape[0], max_frames_per_chunk
                )
